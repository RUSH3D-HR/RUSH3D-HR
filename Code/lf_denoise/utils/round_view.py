...
class RoundView:
    ...
    def __init__(self, radius:int, total_view:int=13) -> None:
        ...
        self.width_per_row = []#每行有多少个视角在mask内
        ...
        self.size = self.mask.sum()#共选取了多少个视角
    ...
    def generate_clockwise_map(self, clockwise=True):
        ...
        stepped = 0
        total = h * w
        d = D.RIGHT if clockwise else D.LEFT
        res = []
        while stepped < total:
            m[u, v] = stepped
            if self.mask[u, v]:
...
def main():
    ...
    lf = np.arange(13 * 13)
    print(select:=np.arange(169).reshape(13,13))
    print(rv.mask)
    print(select[rv.mask])
