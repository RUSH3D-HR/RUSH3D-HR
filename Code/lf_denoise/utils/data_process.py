...
class testset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        xt_volume = xs_volume[:, self.rv.cxs_2_cxt_index_list]
        yt_volume = xt_volume.permute(2, 1, 0)

        xs_volume = xs_volume.unsqueeze(0)
        yt_volume = yt_volume.unsqueeze(0)
        # xs_volume = torch.from_numpy(np.expand_dims(xs_volume, 0).copy())
...
def train_preprocess_lessMemoryMulStacks(args, rv: RoundView):
    ...
    for i in tr:
        ...
        # #when input is whole views:exp(225,405,405), first select the 81 views and rearrange the index into the cycle
        # noise_im = noise_im[rv.lf_2_rv_index_list]
        # noise_im = noise_im[rv.o_2_cxs_index_list]
        #when the input is the cycle indexed views, exp(81,405,405), no need to rearrange the index
...
def test_preprocess(src_img, args,rv: RoundView):
    patch_y = args.patch_y
    patch_x = args.patch_x
    ...
    if noise_im.ndim == 3 and noise_im.shape[0] == 169:
        ...
        noise_im = LF_2_buck(noise_im)
    # #when input is whole views:exp(225,405,405), first select the 81 views and rearrange the index into the cycle
    # noise_im = noise_im[rv.lf_2_rv_index_list][rv.o_2_cxs_index_list].transpose((1, 0, 2))

    noise_im = noise_im.permute(1, 0, 2)
    ...
    for z in range(0, num_s):
        for x in range(0, num_h):
            for y in range(0, num_w):
                ...
                patch_name = '_x' + str(x) + '_y' + str(y) + '_z' + str(z)
