...
import torch.optim
from lf_denoise.lf_denoising.multi_path_abstract import MultiPathAbstract
from lf_denoise.sub_network import SRDTrans
from lf_denoise.lf_denoising.cbam_fusion import ResGroup
from lf_denoise.lf_denoising.rstb_fusion import SwinIR
...
class FusionModule(nn.Module):
    def __init__(self, inplanes=4 * 81, planes=81, stride=1, depths=None, use_residual=False):
        ...
        if depths is None:
            if inplanes % 6 == 0:
                depths_n_heads = [6, 6, 6, 6, 6, 6, 6, 6]#ori
                # depths_n_heads = [6, 6, 6, 6]
                im_size = 48
            elif inplanes % 49 == 0:
                depths_n_heads = [7, 7, 7, 7, 7, 7]
                # depths_n_heads = [7, 7, 7]
